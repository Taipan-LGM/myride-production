#!/usr/bin/env bash
# Smoke test — requires API running on :8001
set -euo pipefail
...
curl -sf "$BASE/ai/suggestions" -H "Authorization: Bearer ***" | python3 -m json.tool | head -30

echo "==> GET /rides/history"
curl -sf "$BASE/rides/history" -H "Authorization: Bearer ***" | python3 -m json.tool | head -20
...
curl -sf "$BASE/payments/ledger" -H "Authorization: Bearer ***" | python3 -m json.tool | head -20

echo "==> POST /channels/voice/simulate"
curl -sf -X POST "$BASE/channels/voice/simulate" \
  -H "Authorization: Bearer ***" \
  -H 'Content-Type: application/json' \
  -d '{"text":"Book a ride from Cape Town CBD to the Waterfront","from_number":"+278****4567"}' \
...
curl -sf -X POST "$BASE/channels/whatsapp/simulate" \
  -H "Authorization: Bearer ***" \
  -H 'Content-Type: application/json' \
  -d '{"text":"Book from Sandton to OR Tambo","from_number":"+278****4567"}' \
  | python3 -m json.tool | head -30
...
curl -sf "$BASE/wallet" -H "Authorization: Bearer ***" | python3 -m json.tool
curl -sf "$BASE/loyalty" -H "Authorization: Bearer ***" | python3 -m json.tool
...
curl -sf "$BASE/driver/earnings" -H "Authorization: Bearer ***" | python3 -m json.tool | head -25
